...
public class ClassService : IClassService
{
    ...
    public async Task<ClassProfileResponse> GetClassProFile(ClassProfileRequest model)
    {
        ...
        {
            var t = await _context.classMemberMapping.AnyAsync(x =>
                x.UserId == model.UserId && x.ClassId == model.ClassId
            );
            if (!t)
                return new ClassProfileResponse
                {
                    Status = "failure",
                    Message = "You do not in this class"
                };
            var ClassModel = await _context.classes.FirstOrDefaultAsync(x => x.Id == model.ClassId);
